...
public class ExifWrapper {
    ...
    /**
     * The full set of GPS-related EXIF tags, per the IIM/EXIF GPS IFD - used by copyGpsExif to
     * recover the whole GPS dictionary (altitude, speed, track, timestamps, destination
     * coordinates, etc.), not just latitude/longitude.
     */
    private final String[] gpsAttributes = new String[] {
        TAG_GPS_ALTITUDE,
    ...
    /**
     * Copies only the GPS-related EXIF tags (see gpsAttributes) from this wrapper's source onto
     * destFile, leaving every other tag on destFile untouched. Use this instead of copyExif when
     * destFile's non-GPS metadata (dimensions, orientation, etc.) may have legitimately changed
     * relative to the source - e.g. after a resize or orientation correction - and copying those
     * over unconditionally would make destFile's metadata inconsistent with its actual content.
     */
    public void copyGpsExif(String destFile) {
        try {
            ExifInterface destExif = new ExifInterface(destFile);
            for (int i = 0; i < gpsAttributes.length; i++) {
                String value = exif.getAttribute(gpsAttributes[i]);
                if (value != null) {
                    destExif.setAttribute(gpsAttributes[i], value);
                }
            }
            destExif.saveAttributes();
        } catch (Exception ex) {}
